  ...
  return (
    <div className="flex flex-col h-full bg-[#f8fafc] overflow-y-auto font-sans pb-28 text-slate-900 relative">
      ...
      <div className="p-5 space-y-4">
        ...
        {/* Akıllı Havacılık Biniş & Trim (Ağırlık-Denge) Optimizasyon Sistemi */}
        <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm text-left space-y-4">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h3 className="text-slate-900 text-[11px] font-black uppercase tracking-wider">
                  ✈️ Uçuş Biniş & Trim (Ağırlık-Denge) Analizörü
                </h3>
                <p className="text-[9.5px] text-slate-400 font-medium">IATA AHM 560 Havacılık Standartlarına Uyumlu Yerleşim Sistemi</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-1.5">
              {[
                ...
                >
                  ...
                  <span className={`text-[8px] leading-snug mt-1 ${trimStrategy === strat.id ? "text-indigo-200" : "text-slate-450"}`}>
                    {strat.desc}
                  </span>
                </button>
              ))}
          ...
          <div className="bg-slate-50 p-4.5 rounded-2xl border border-slate-150 space-y-3.5">
                ...
                return (
                  <>
                    ...
                    <div className="flex justify-between text-[8px] text-slate-400 font-mono mt-2 pt-1 uppercase">
                      <span className="text-rose-600 font-bold">BURUN-AĞIR Sınır (48%)</span>
                      <span className="text-emerald-700 font-bold">İdeal Trim Alanı (%50 - %53)</span>
                      <span className="text-rose-600 font-bold font-mono">ARKA Trim Sınır (56%)</span>
                    </div>
                  </>
                );
              })()}
            ...
            {(() => {
              ...
              if (cgVal > 56.0) {
                return (
                  <div className="bg-rose-50/70 border border-rose-200 rounded-xl p-3 flex gap-2.5 items-start">
                    ...
                    <div className="text-[9.5px] leading-relaxed text-rose-800">
                      <strong className="font-extrabold uppercase block text-rose-950">🚨 TRİM KİLİDİ UYARISI: AFT LİMİT MODEL AŞILDI (%{cgVal} MAC)</strong>
                      Yolcular sadece arkadan öne bindiği için uçağın kuyruk ağırlığı tavan yaptı! Kuyruk üzerine çökme riski (Tail strike / tip-over) bulunmaktadır. Havayolu yer personelinin acilen burnu dengelemesi gereklidir.
                    </div>
                  </div>
                );
              }
                ...
                return (
                  <div className="bg-amber-50/80 border border-amber-200 rounded-xl p-3 flex gap-2.5 items-start">
                    ...
                    <div className="text-[9.5px] leading-relaxed text-amber-850">
                      <strong className="font-extrabold uppercase block text-amber-950">⚠️ TRİM UYARISI: FORWARD LİMİT AŞILDI (%{cgVal} MAC)</strong>
                      Uçağın ön tarafı aşırı yüklü! Kalkış anında (take-off rotation) tırmanış açısı dengesizliği veya burun iniş takımı aşırı stres yüklemesi yaşanabilir.
                    </div>
                  </div>
                );
              }
              return (
                <div className="bg-emerald-50/70 border border-emerald-150 rounded-xl p-3 flex gap-2.5 items-start">
                  ...
                  <div className="text-[9.5px] leading-relaxed text-emerald-800">
                    ...
                    Ağırlık merkezi, havacılık güvenlik marjları içinde mukemmel şekilde dağılmış durumda. Yakıt tasarrufu ve aerodinamik kararlılık optimize edilmiştir.
                  </div>
                </div>
              );
            })()}
          ...
          <div className="space-y-2.5">
            <div className="flex justify-between items-center text-[10px]">
              ...
              <span className="font-mono bg-slate-100 font-bold px-1.5 py-0.5 rounded text-slate-650">
                Uygulanan Adım: {boardingStep}/4
              </span>
            </div>

            <div className="grid grid-cols-4 gap-1 text-center font-semibold text-[8px] tracking-wide relative">
              {[
            ...
            <div className="space-y-1.5 max-h-56 overflow-y-auto border border-slate-100 rounded-xl p-2.5 bg-slate-50/30">
              {paxQueue.map((p) => (
                <div 
                  key={p.id} 
                  className={`p-2.5 rounded-xl border flex justify-between items-center transition-all ${
                    p.status === "Boarded" 
                      ? "bg-emerald-50/60 border-emerald-150 text-slate-800 shadow-3xs" 
                      : "bg-white border-slate-200 text-slate-600"
                  }`}
                >
                  <div className="space-y-1 text-[9.5px] truncate max-w-[80%]">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      {p.isWchr && <span className="bg-blue-100 text-blue-700 text-[7px] font-black px-1 rounded border border-blue-250 uppercase">WCHR Koridoru</span>}
                      {p.isLoyalty && <span className="bg-amber-150 text-amber-900 text-[7px] font-black px-1 rounded border border-amber-250 uppercase">VIP Öncelikli</span>}
                      <strong className="text-slate-850 font-semibold">{p.name}</strong>
                    ...
                    <span className="text-[8.5px] text-slate-400 block truncate font-medium">{p.description}</span>
                  </div>
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-150 disabled:text-slate-400 disabled:border-slate-200 border border-indigo-600 text-white font-extrabold text-[10.5px] tracking-wide py-3 px-4 rounded-xl cursor-pointer hover:scale-[1.01] active:scale-95 shadow-sm transition-all text-center focus:outline-none"
              >
                {boardingStep >= 4 ? "BİNİŞ TAMAMLANDI ✓" : "SIRADAKİ GRUBU BİNDİR (+1 ADIM)"}
              </button>
              <button
                onClick={handleResetBoardingSim}
                className="bg-slate-100 hover:bg-slate-200 border border-slate-250 text-slate-700 font-bold text-[10.5px] py-3 px-4 rounded-xl cursor-pointer active:scale-95 transition-all text-center focus:outline-none"
              >
                SIFIRLA
              </button>
            </div>
